...
function EditImage() {
    ...
    //Redirect if user did not post listing
    useEffect(() => {
        if(listing && listing.userRef !== auth.currentUser.uid) {
            toast.error('Sorry, you do not own this listing and cannot edit it')
            navigate('/')
        }
    })
    ...
    const onSubmit = async(e) => {
        ...
        if(geolocationEnabled) {
            const response = await fetch(`https://maps.googleapis.com/maps/api/geocode/json?address=${address}&key=${process.env.REACT_APP_GEO_APIKEY}`)
